    ...
    module Matchers
      ...
      class IncludeInstanceMethod
        ...
        def matches? klass
          @klass = klass
          if RUBY_VERSION =~ /1\.9/
            klass.instance_methods.include?(@meth.to_sym)
          else
      ...
      class IncludeClassMethod
        ...
        def matches? klass
          @klass = klass
          if RUBY_VERSION =~ /1\.9/
            klass.methods.include?(@meth.to_sym)
          else
            klass.methods.include?(@meth.to_s)
          end
        end
